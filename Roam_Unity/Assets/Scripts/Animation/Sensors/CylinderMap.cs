...
public class CylinderMap {
	...
	private void Generate() {
		float diameter = GetDiameter(); //0.5
		List<Vector3> points = new List<Vector3>();
		List<float> radius = new List<float>();
		for (int y=0; y<Layers; y++) {//9
			for(int z=0; z<Resolution; z++) {//9
				float coverage = 0.5f * diameter; //0.25
				float distance = (float)z * coverage; //0 -> 0.25 * 9
				float arc = 2f * Mathf.PI * distance;
				int count = Mathf.RoundToInt(arc / coverage);// arc * 4
				//Debug.Log(count);
