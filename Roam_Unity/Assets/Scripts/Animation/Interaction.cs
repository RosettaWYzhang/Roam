...
public class Interaction : MonoBehaviour {
	...
	void Awake() {
		if(Application.isPlaying) {
			...
			if (GetGeometry() == null){
				Debug.Log("No coliider is found");
			}
	...
	void OnRenderObject() {
		if(ShowContacts) {
			...
			for(int i=0; i<Contacts.Length; i++) {
				if(Contacts[i] != null) {
					UltiDraw.DrawSphere(Contacts[i].position, Contacts[i].rotation, 0.1f, colors[i]);
					UltiDraw.DrawTranslateGizmo(Contacts[i].position, Quaternion.LookRotation(Contacts[i].rotation.GetForward(), Vector3.up), 0.2f);
