...
public class Actor : MonoBehaviour {
	...
	public bool DrawGoalJointTransform = false;
	...
	public void SetBoneLocalTransformation(Matrix4x4 value, string bone) {
		Bone b = FindBone(bone);
		if(b != null) {
			b.Transform.localPosition = value.GetPosition();
			b.Transform.localRotation = value.GetRotation();
		}
	}
	...
	public void Draw(Color boneColor, Color jointColor, float alpha) {
		...
		if(DrawSkeleton) {
			...
			recursion = new Action<Bone>((bone) => {
                if(!bone.GetName().Contains("Thumb") && !bone.GetName().Contains("Index") && !bone.GetName().Contains("Middle") && !bone.GetName().Contains("Ring") && !bone.GetName().Contains("Pinky")){
                    if (bone.GetParent() != null)
                    {
                        UltiDraw.DrawBone(
                            ...
                            //Quaternion.FromToRotation(bone.GetParent().Transform.up, bone.Transform.position - bone.GetParent().Transform.position) * Quaternion.LookRotation(bone.GetParent().Transform.rotation.GetUp(), bone.GetParent().Transform.rotation.GetForward()),
		...
		if(DrawGoalJointTransform) {
			Action<Bone> recursion = null;
			recursion = new Action<Bone>((bone) => {
				UltiDraw.DrawWiredSphere(bone.Transform.position, GetRoot().rotation, 0.1f, UltiDraw.White, UltiDraw.Black);
				UltiDraw.DrawTranslateGizmo(bone.Transform.position, GetRoot().rotation, 0.15f);
				//UltiDraw.DrawTranslateGizmo(bone.Transform.position, bone.Transform.rotation, 0.05f);
				for(int i=0; i<bone.Childs.Length; i++) {
					recursion(bone.GetChild(i));
				}
	...
	public class Actor_Editor : Editor {
		...
		public override void OnInspectorGUI() {
            ...
            Target.DrawGoalJointTransform = EditorGUILayout.Toggle("Draw Goal Joint Transforms", Target.DrawGoalJointTransform);
