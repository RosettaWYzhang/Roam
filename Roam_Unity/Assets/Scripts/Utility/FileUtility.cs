...
public static class FileUtility {
	...
	public static float[] ReadNthLineFromTextFile(string fn, int colSize, int index)
    {
        string line = File.ReadLines(fn).ElementAt(index);
        float[] data = new float[colSize];
        string[] currlinedata = line.Split(' ');
        for (int j = 0; j < colSize; j++)
        {
            data[j] = float.Parse(currlinedata[j]);
        }
        return data;
    }
