...
public class Frame {
	...
	public Vector3[] GetBonePositions(bool mirrored) {
		Matrix4x4[] transformations = GetBoneTransformations(mirrored);
		Vector3[] positions = new Vector3[transformations.Length];
	...
	public Matrix4x4 GetBoneTransformation(int index, bool mirrored) {
		...
		//only scaling the translation here
		m[0, 3] = Data.Scale * (m[0, 3] + o.x);
		m[1, 3] = Data.Scale * (m[1, 3] + o.y);
		m[2, 3] = Data.Scale * (m[2, 3] + o.z);
		// comment or uncomment this for NSM/our dataset
		// m = permute * m;
        // // swapping forward and upward columns
        Vector3 forward = m.GetForward();
        Vector3 up = m.GetUp();
        m.SetColumn(1, new Vector4(forward[0], forward[1], forward[2], 0f));
        m.SetColumn(2, new Vector4(up[0], up[1], up[2], 0f));
		return m;
	}
