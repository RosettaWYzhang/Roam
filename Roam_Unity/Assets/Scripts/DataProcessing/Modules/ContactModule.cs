...
public class ContactModule : Module {
	...
	protected override void DerivedDraw(MotionEditor editor) {
		...
		if(ShowContacts) {
			for(int i=0; i<Sensors.Length; i++) {
				if(Sensors[i].Edit != Sensor.ID.None) {
					// don't understand the for loop here 0> why visualization multiple frames?
		...
		public void CaptureContact(Frame frame, MotionEditor editor) {
			...
			Frame[] frames = Module.Data.GetFrames(Mathf.Clamp(frame.Index-width, 1, Module.Data.GetTotalFrames()), Mathf.Clamp(frame.Index+width, 1, Module.Data.GetTotalFrames()));
			{
				...
				for(int i=0; i<frames.Length; i++) {
                    ...
                    // get pivot is calling offset.GetRelativePositionFram the bone transformation
                    // GetRelativePositionFrom: from.MultiplyPoint3x4(position)
                    Vector3 pivot = editor.Mirror ? GetPivot(f, false).GetMirror(f.Data.MirrorAxis) : GetPivot(f, false);
					// pivot should be 0 if we set offset to 0
					// TODO: verify
					Collider collider;
				...
				// contacts is an array of boolean of length |frames|
				// consider a window of frames, so that the estimation is more stable
			...
			}
			{
				...
				for(int i=0; i<frames.Length; i++) {
					...
					Collider collider;
					// the collider did not consider velocity
			...
			if(Velocity > 0f) {
				// second argument is whether it's mirrored
				if(GetContact(frame, false) == 1f) {
					// if in contact and the bone has a velocity bigger than a threshold
					// we set contact to zero
					if(frame.GetBoneVelocity(Bone, false, 1f/Module.Data.Framerate).magnitude > Velocity) {
						RegularContacts[frame.Index-1] = 0f;
						// GetPivot: get relative distance between offset and bone
