...
public class PhaseModule : Module {
	...
	protected override void DerivedInspector(MotionEditor editor) {
		...
		using(new EditorGUILayout.VerticalScope ("Box")) {
			...
			if (Utility.GUIButton("Estimate Phase", UltiDraw.DarkGrey, UltiDraw.White))
			{
				EditorCoroutines.StartCoroutine(CapturePhase(editor), this);
			}
	...
	private IEnumerator CapturePhase(MotionEditor editor)
	{
		RegularPhaseFunction.Clear();
		InversePhaseFunction.Clear();
		System.DateTime time = Utility.GetTimestamp();
		int max_frame = 100;
		ContactModule contact = (ContactModule)Data.GetModule(ID.Contact);
		ContactModule.Sensor leftfoot = contact.GetSensor("LeftFoot");
		ContactModule.Sensor rightfoot = contact.GetSensor("RightFoot");
		float leftInContact = 0f;
		float rightInContact = 0f; 
		int prevLeftKey = 0;
		int prevRightKey = 0;

		for (int i = 0; i < Data.Frames.Length; i++)
		{
			Frame curr_frame = Data.GetFrame(i);
			float currLeftContact = leftfoot.GetContact(curr_frame, false);
			float currRightContact = rightfoot.GetContact(curr_frame, false);

			// estimate inverse contact
			if (currRightContact > 0f && rightInContact == 0f  || (i - prevRightKey) > max_frame)
			{
				rightInContact = 1f;
				InversePhaseFunction.Keys[i] = true;
				InversePhaseFunction.Phase[i] = 1f;
				// interpolate from previous key
				for (int j = prevRightKey+1; j < i; j++)
				{
					InversePhaseFunction.Phase[j] = (float)(j - prevRightKey) / (float)(i - prevRightKey);
				}
				prevRightKey = i;

			}
			else if (rightInContact == 1f && currRightContact == 0f)
			{
				rightInContact = 0f;
			}


            // estimate regular phase
            if (leftInContact == 0f && currLeftContact == 1f || (i - prevLeftKey) > max_frame)
            {

                leftInContact = 1f;
                RegularPhaseFunction.Keys[i] = true;
                RegularPhaseFunction.Phase[i] = 1f;


                for (int j = prevLeftKey+1; j < i; j++)
                {

                    RegularPhaseFunction.Phase[j] = (float)(j - prevLeftKey) / (float)(i - prevLeftKey);
                }
                prevLeftKey = i;
            }
            else if (leftInContact == 1f && currLeftContact == 0f)
            {
                leftInContact = 0f;
            }
	...
	public class PhaseEvolution {
		...
		public class Population {
			...
			public void Evolve(Population previous, Population next, Population previousPivot, Population nextPivot) {
				if(Active) {
					...
					for(int o=1; o<Size; o++) {
						...
						if(UnityEngine.Random.value <= Evolution.Behaviour) {
							...
							for(int i=0; i<Dimensionality; i++) {
								...
								float momentum = UnityEngine.Random.value * parentA.Momentum[i] + UnityEngine.Random.value * parentB.Momentum[i];
								...
								if(UnityEngine.Random.value <= mutationRate) {
									...
									offspring.Genes[i] += UnityEngine.Random.Range(-mutationStrength*span, mutationStrength*span);
								}
								
								//Adoption
								weight = UnityEngine.Random.value;
								offspring.Genes[i] += 
									weight * UnityEngine.Random.value * (0.5f * (parentA.Genes[i] + parentB.Genes[i]) - offspring.Genes[i])
									+ (1f-weight) * UnityEngine.Random.value * (prototype.Genes[i] - offspring.Genes[i]);
			...
			private void Reroll(Individual individual) {
				for(int i=0; i<Dimensionality; i++) {
					individual.Genes[i] = UnityEngine.Random.Range(Evolution.LowerBounds[i], Evolution.UpperBounds[i]);
				}
			}
			...
			private void Exploit(Individual individual) {
				...
				for(int i=0; i<Dimensionality; i++) {
					...
					float incGene = Mathf.Clamp(gene + UnityEngine.Random.value*individual.Fitness*span, Evolution.LowerBounds[i], Evolution.UpperBounds[i]);
					individual.Genes[i] = incGene;
					float incFitness = ComputeFitness(individual.Genes);

					float decGene = Mathf.Clamp(gene - UnityEngine.Random.value*individual.Fitness*span, Evolution.LowerBounds[i], Evolution.UpperBounds[i]);
			...
			private Individual Select(Individual[] pool) {
				double rVal = UnityEngine.Random.value * RankProbabilitySum;
				for(int i=0; i<Size; i++) {
