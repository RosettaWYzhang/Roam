...
def train(model, train_dataloader, epochs, lr, steps_til_summary, epochs_til_checkpoint, model_dir, loss_fn,
    ...
    total_steps = 0
    with tqdm(total=len(train_dataloader) * epochs, miniters=int(len(train_dataloader) * epochs/100)) as pbar:
        train_losses = []
                ...
                if not total_steps % steps_til_summary and rank == 0:
                    ...
                    if val_dataloader is not None:
                        ...
                        for loss_name, loss in val_losses.items():
                            ...
                            print("Validation loss is %f" %single_loss)
                        model.train()
...
def train_feature(model, train_dataloader, corr_model, epochs, lr, steps_til_summary, epochs_til_checkpoint, model_dir, loss_fn,
    ...
    total_steps = 0
    with tqdm(total=len(train_dataloader) * epochs, miniters=int(38100/100)) as pbar:
        train_losses = []
        for epoch in range(epochs):
            if not epoch % epochs_til_checkpoint and epoch and rank == 0:
                torch.save(model.state_dict(),
            ...
            for step, (model_input, gt) in enumerate(train_dataloader):
                ...
                if not total_steps % steps_til_summary and rank == 0:
                    print("Epoch %d, Train Total loss %0.6f, iteration time %0.6f" % (epoch, train_loss, time.time() - start_time))
                    if val_dataloader is not None:
                        print("Running validation set...")
                        with torch.no_grad():
                            val_losses = defaultdict(list)
